...
A [DimOS](https://github.com/dimensionalOS/dimos)-based drone room mapping project, based on **vision instead of lidar**. 
A depth camera feed is projected into a 3D point cloud and fed into the same mapping/exploration stack that would normally process lidar data, 
validated inside [DimSim](https://github.com/Antim-Labs/DimSim) (a browser-based robot simulator) against DimSim's own ground-truth pose before any of it is trusted on the real drone.

## Setup

- Python 3.12
- `uv sync` (or `pip install -e .`) from this directory
- A [DimOs fork](https://github.com/knikou01/dimos) modified to support 2D drone room mapping
- A [DimSim fork](https://github.com/knikou01/DimSim/tree/fix/dimos-yaw-sync)

## Visual SLAM in DimSim

`blueprints/sim_dimsim_vo_blueprint.py` builds a world-frame point cloud from DimSim's depth camera image and feeds it to `VoxelGridMapper` instead of DimSim's own raycast lidar topic.
Mapping currently uses DimSim's ground-truth pose, not a vision-based pose estimate — see **Known limitations** below.

### Prerequisites (both modes below)

1. Start the DimSim relay:
   In the DimSim directory:
   ```bash
   deno run --allow-all --unstable-net dimos-cli/cli.ts dev --scene apt
   ```
2. Open `http://localhost:8090/?dimos=1&scene=apt` in a browser and wait for `[dimos] Bridge connected. Sensor publishing active.` in the browser's console. Do this before starting either script below.

### Autonomous exploration

Drives the robot itself via a classical frontier-exploration + A* planner stack (`WavefrontFrontierExplorer` + `ReplanningAStarPlanner` + `MovementManager`), building the map as it goes.
...
For moving the simulation drone manually — useful for testing visual odometry
...
The LLM's role is request-driven: start/stop exploration, drive the drone directly when
...
seconds and then turns it to face the opposite direction, so the camera isn't
...
### VO-driven navigation (experimental)
...
- **Visual odometry (VO) is not yet accurate enough to trust for real
  navigation, and mapping stays on ground truth.** 
  Swapping DimOS' pose source from ground truth to real
  VO made progress but did not reach a reliable end
  state. See `drone/dimsim_visual_odometry_module.py`'s module docstring
  for the full evidence trail.
  - **"VO barely moves at all":** its
    image-capture path did a synchronous, GPU-pipeline-stalling pixel
    readback on every frame ("GPU stall due to ReadPixels" driver
    ...
    Fixed in a [DimSim fork](https://github.com/knikou01/DimSim/tree/fix/dimos-yaw-sync) (`engine.js`/`dimosBridge.ts`) by switching to
    ...
    though it can still degrade to several seconds under heavier load.
  - **`DimSimVisualOdometryModule` was rewritten to track via ORB feature
    matching + depth-based 3D-3D RANSAC** (`tracking_method="sparse"`) instead of dense frame-to-frame photometric
    ...
    Under real (non-best-case) load, dense 
  ...
  - **Even sparse tracking has a low real-world success rate against
    ...
    testing), mainly 
    because many views don't offer enough distinctive, well-
    distributed visual texture for reliable feature matching in the first
    place. This is a content/rendering
  ...
  - **A third tracking method, `tracking_method="learned"`, is now
    ...
    features are more robust than ORB's hand-crafted ones on
    low-texture/repetitive scenes.
    This approach is way more expensive per frame than ORB and runs on CUDA
    ...
    tracking method needs a GPU. It's not the default, opt in explicitly.
  - **Decision: decouple navigation from mapping rather than swap both
    ...
    `ReplanningAStarPlanner`'s frequent replanning. So two
    blueprints (`blueprints/sim_dimsim_vo_nav_blueprint.py`,
    ...
    `DimSimDepthLidarModule` on ground truth. These are still experimental. With VO's current success rate, the
...
- **Depth-camera mapping has narrower, heading-dependent coverage than
  ...
  through.** DimOS's A* planner (`min_cost_astar.py`) treats unmapped
  ...
  has to be willing to path toward space it hasn't seen yet. If a wall segment was never observed closely enough by the
  ...
  nearby wall almost immediately regardless of heading. This is a tradeoff of vision-only mapping
  that can't be fixed using only visual SLAM.
