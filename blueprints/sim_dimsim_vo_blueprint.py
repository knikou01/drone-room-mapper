...
Confirmed-working precedent this builds on (per Vector's technical report,
section 1.3): WavefrontFrontierExplorer + ReplanningAStarPlanner +
MovementManager already drive real autonomous exploration against DimSim's
own ground-truth /odom, with no remapping needed for odom itself.

CostMapper is required here -- omitting it was a real bug in an earlier
version of this file: without it, WavefrontFrontierExplorer.latest_costmap
is never populated, so its exploration loop silently waits forever without
publishing any goal or logging anything (confirmed from its source: `if
self.latest_costmap is None ... continue`, no log line). This caused the
drone to never move at all during initial Phase 1 testing.

Prerequisite: DimSim relay running with a scene loaded and a browser tab
connected (see Vector's report section 1.1) BEFORE this blueprint starts.
...
KNOWN ISSUE, NOT PART OF THIS TASK -- DO NOT ATTEMPT TO FIX (2026-07-08):
the "apt" scene (currently the only scene defined in this DimSim fork's
...
body contacts it, the robot gets permanently wedged -- confirmed NOT
recoverable even via manual teleop through the localhost:7779 dashboard.
This is a known, general limitation of robots running DimOS against
DimSim's thin/transparent-surface collision handling, not specific to this
blueprint or to VO -- per the user, may be worth revisiting for the agentic
exploration version, but out of scope here. Workaround for testing VO under
real sustained motion: do NOT trigger python -m lcm_probe.start_exploration
(which is what drives the pathfinder into the glass) -- instead drive the
robot entirely by hand via the localhost:7779 teleop dashboard, staying
clear of the glass, and let it run long enough to accumulate real motion
for VO-vs-ground-truth comparison.
"""
...
sim_dimsim_vo = autoconnect(
    ...
    [
        ...
        # reasoning. Remapping syntax confirmed from
        # dimos/robot/drone/blueprints/agentic/drone_agentic.py's existing
        # (DroneTrackingModule, "video_input", "video") pattern.
